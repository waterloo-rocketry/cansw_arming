...
void indicator_buzzer_heartbeat(void){
    ...
    if((uint16_t)(ADCC_GetSingleConversion(channel_BATTERY_1))*ANALOG_SCALLER > UNDERVOLTAGE_THRESHOLD && loop_time < 750) 
        BUZZER_ON();
    
    else if(loop_time >= 750 && loop_time < 1000) BUZZER_OFF();
    
    else if((uint16_t)(ADCC_GetSingleConversion(channel_BATTERY_2))*ANALOG_SCALLER > UNDERVOLTAGE_THRESHOLD && loop_time >= 1000 && loop_time < 1250) 
        BUZZER_ON();
    
    else if(loop_time >= 1250 && loop_time < 1500) 
        BUZZER_OFF();
    
    else if((uint16_t)(ADCC_GetSingleConversion(channel_BATTERY_2))*ANALOG_SCALLER > UNDERVOLTAGE_THRESHOLD && loop_time >= 1500 && loop_time < 1750) 
        BUZZER_ON();
