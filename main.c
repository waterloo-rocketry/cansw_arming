...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		if (millis() >= last_millis + MAX_LOOP_TIME_DIFF_ms) {
			...
			build_general_board_status_msg(PRIO_MEDIUM, millis(), status_bitfield, &board_stat_msg);
			...
			if (BOARD_INST_UNIQUE_ID == BOARD_INST_ID_ARMING_RA_RAVEN) {
				...
				build_analog_sensor_16bit_msg(PRIO_MEDIUM,
											  millis(),
			...
			if (new_altitude_available()) {
				...
				build_analog_sensor_16bit_msg(
					...
					SENSOR_ALTITUDE,
					get_altitude(),
					&altitude_msg); // TODO: Replace SENSOR_PAYLOAD_INFRARED with actual altimeter
									// sensor
