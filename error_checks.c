...
bool check_battery_voltage_overvoltage(void) {
	...
	// uint16_t batt2_voltage = (uint16_t)ADCC_GetSingleConversion(channel_BATTERY_2) *
	// ANALOG_SCALAR;
...
void indicator_buzzer_heartbeat(void) {
	...
	if (mag1_active()) {
		for (uint8_t i = 0; i < buzzer_beep_count; ++i) {
			...
			if (beep_start_time <= time_in_current_loop &&
				time_in_current_loop < beep_start_time + buzzer_beep_ms) {
